...
> **靶点**：PD-L1 (CD274)  
> **当前进度**：Module 0–4 ✅ · Module 6 可视化 ✅ · Module 5（MM-GBSA）规划中 / RunPod GPU
...
flowchart LR
    ...
    subgraph M4_5["Module 4 ✅ · Module 5 🔜"]
        M4[Module 4<br/>AF3 Server 解析]
        M5[Module 5<br/>HawkDock / MM-GBSA]
        M3 --> M4 --> M5
    end
...
| Module 5（规划） | 结合自由能无优势、能量分解不合理 | ΔG_bind / 热点残基贡献阈值（待定义） |
...
| 可视化 | matplotlib、seaborn、py3Dmol | DMS 热图、漏斗、AF3 排名 / 3D HTML |
| 结构验证 | AlphaFold 3 Server + 本地解析 | 纳米抗体–PD-L1 复合物；ipTM / pTM |
| 自由能（规划） | HawkDock / MM-GBSA | 结合自由能分解（RunPod GPU） |
| 运行环境 | Colab CPU + AF3 Server（半自动） | Module 0–3、6 CPU；Module 4 Server；Module 5 GPU |
...
- **Module 5 未实现**：HawkDock / MM-GBSA 需 RunPod GPU。
...
Module 4  →  解析 AF3 → af3_pe_metrics.csv + fig_pe_af3_*
    ↓
Module 6  →  生成 figures/fig5*、fig6*（7 张 PNG）
```

> **注意**：Module 5（MM-GBSA）需 RunPod GPU，当前仍为占位。Module 4 已可解析本地 AF3 结果。
...
└── screened_results/
    ...
    └── figures/fig5*.png, fig6*.png  # Module 6
```
...
### Module 5 — 结合自由能解析（规划中）

**目的**：基于 Module 4 复合物结构，用 HawkDock / MM-GBSA 估算结合自由能并分解至残基贡献。

**前置依赖**：Module 4 复合物 PDB。

**预期输出**：`ppi_energy_decomposition.csv`

**算力**：**RunPod GPU**（推荐 A100 / RTX 4090）。

**可迁移场景**：抗体工程、亲和力成熟后的能量归因分析。

---

### Module 6 — 可视化与结果导出

**目的**：将 Module 2–3 数据汇总为发表级图表；Module 4 专图由 Module 4 cell 直接写出。

**前置依赖**：`mutation_scores.csv`、`ppi_docking_scores.csv`。
...
flowchart TB
    ...
    subgraph M4["Module 4 ✅"]
        Fpe[fig_pe_af3_* ipTM / 3D]
    end
    subgraph M5["Module 5 规划"]
        F7[能量雷达 / MM-GBSA]
    end
    M2 --> F5a & F5b & F5c & F5d
    M3 --> F6a & F6b & F6c
...
| 6a–6c | `fig6a_*` … `fig6c_*` | Module 3 → 解读见 [Module 3](#module-3--蛋白-蛋白对接与界面评分) |
| PE-AF3 | `fig_pe_af3_*` | Module 4 → 解读见 [Module 4](#结果解读module-4-可视化) |
| 规划 | MM-GBSA 能量图 | Module 5 |
...
| 项目 | 说明 |
|------|------|
| **图意** | 三阶段：CDR3 突变数 → Top-20 对接 → ΔLL>0 有益突变数 |
| **本数据结论** | 361 → 20 → 有益突变子集，漏斗反映序列阶段收窄 |
| **含义与局限** | Module 4 已补充 AF3 相对排序；Module 5 完成后可增加「ΔG 优选」阶段 |

> **关于 3D 结合图**：纳米抗体–PD-L1 全原子 3D 见 Module 4 的 `fig_pe_af3_complex.html`；**DNA/RNA 结合 3D 属于 NA 线路**。
...
### 4.4 `nanobody_cdr_regions.json`（Module 1）
...
| `fig6a`–`fig6c` | PPI 界面与筛选漏斗（Module 3） |
| `fig_pe_af3_*` | AF3 纳米抗体–PD-L1（排名 / 3D） |
| Module 5 规划 | MM-GBSA 能量分解图 |
...
```text
Colab Module 0–3 完成
    ↓ export_for_local_sync() 或临时文件床打包下载
本地 data/
    ↓ git / scp 上传
RunPod 实例
    ↓ Module 4（AF3 结果下载）→ Module 5（MM-GBSA）
ppi_energy_decomposition.csv
```
...
| 文件 | 必需 |
|------|------|
| `data/receptor/PDL1_4ZQK_chainB.pdb` | ✅ |
| `data/screened_results/mutation_scores.csv` | ✅ |
| `data/screened_results/ppi_docking_scores.csv` | ✅ |
| `data/screened_results/pe_docking/*.pdb` | ✅（Top N 结构） |
| `data/raw_libraries/nanobody_cdr_regions.json` | ✅ |
| AF3 复合物 CIF | ✅ `af3_pe_complexes/*_best.cif` |
| `af3_pe_metrics.csv` | ✅ |
...
| **NA** | 核酸 | siRNA / Aptamer 序列 | *待补充* |

三条线路共享 `data/receptor/` 下的 PD-L1 结构资源，但筛选逻辑、输出 CSV 与可视化图号独立（SM 用 fig3/fig4，PE 用 fig5/fig6）。

---

*文档版本：2026-07 · Module 0–4、6 已实现 · Module 5（MM-GBSA）规划中*
